...
let studentImgEl = document.querySelector("#student-image");
...
const studentsWithShuffledAnswers = shuffledArray(students).map((item) => {
  const notCorrect = shuffledArray(students)
    .filter((student) => student.id !== item.id)
    .splice(0, 3);
...
const renderNewQuestion = () => {
  ...
  studentImgEl.src = currentQuestion.image;
};
...
const startGame = () => {
  ...
  //shuffledArray(studentsWithShuffledAnswers);
};
//--------------------------------------------------------------------------
...
const getHeader = () => {
  h1El.textContent = "How well do you know your classmates? ";
};
...
document.querySelector(".btn-10").addEventListener("click", () => {
  totalQuestions = shuffledArray(studentsWithShuffledAnswers).slice(0, 10);
  renderNewQuestion();
  startGame();
});

document.querySelector(".btn-20").addEventListener("click", () => {
  totalQuestions = shuffledArray(studentsWithShuffledAnswers).slice(0, 20);
  renderNewQuestion();
  startGame();
});

document.querySelector(".btn-30").addEventListener("click", () => {
  totalQuestions = shuffledArray(studentsWithShuffledAnswers).slice(0, 30);
  renderNewQuestion();
  startGame();
});

document.querySelector(".btn-all").addEventListener("click", () => {
  totalQuestions = shuffledArray(studentsWithShuffledAnswers);
  renderNewQuestion();
  startGame();
});
